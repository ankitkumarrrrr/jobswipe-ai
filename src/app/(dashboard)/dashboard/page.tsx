...
const statusColors: Record<string, string> = {
  ...
  VIEWED: "bg-purple-100 text-purple-700 dark:bg-purple-500/15 dark:text-purple-400",
  RESPONDED: "bg-green-100 text-green-700 dark:bg-green-500/15 dark:text-green-400",
  INTERVIEW: "bg-emerald-100 text-emerald-700 dark:bg-emerald-500/15 dark:text-emerald-400",
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard title="Applications Sent" value={String(totalApps)} change={`${totalApps > 0 ? "Active" : "Start swiping!"}`} changeType="positive" icon={Send} iconColor="text-blue-500" iconBg="bg-blue-100 dark:bg-blue-500/10" />
            ...
            {monthlyData.every(d => d.apps === 0) ? (
              ...
              <>
                  ...
                  {monthlyData.map((d, i) => (
                    <div key={i} className="flex-1 flex flex-col items-center gap-2">
                      <div className="w-full flex flex-col items-center gap-1">
                        <div className="w-full bg-blue-500 rounded-t-md" style={{ height: `${maxH > 0 ? (d.apps / maxH) * 140 : 0}px` }} />
                ...
                <div className="flex items-center justify-center gap-6 text-xs mt-4">
                  <div className="flex items-center gap-2"><div className="h-3 w-3 rounded-sm bg-blue-500" /><span className="text-gray-500 dark:text-gray-400">Applications</span></div>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="text-center">
              ...
              {subscriptionPlan === "FREE" && <Link href="/settings"><Button className="w-full bg-blue-600 hover:bg-blue-700 text-white text-sm">Upgrade Plan</Button></Link>}
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <Card>
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            ...
            <Link href="/applications" className="text-sm text-blue-500 hover:text-blue-600">View All</Link>
          </CardHeader>
          <CardContent>
            {recentApps.length === 0 ? (
              <div className="text-center py-8">
                ...
                <Link href="/jobs"><Button size="sm" className="mt-3 bg-blue-600 hover:bg-blue-700 text-white">Find Jobs</Button></Link>
              </div>
            ) : (
              <div className="space-y-4">
                {recentApps.map((app: any) => (                    <div key={app.id} className="flex items-center gap-4 p-3 rounded-lg hover:bg-gray-50 dark:hover:bg-white/5 transition-colors">
                    <div className="h-10 w-10 rounded-lg bg-blue-100 dark:bg-blue-500/10 flex items-center justify-center text-sm font-bold text-blue-600 dark:text-blue-400">{(app.company || "?")[0]}</div>
                    ...
                    <div className="flex items-center gap-2">
                      <Badge className="text-xs bg-blue-100 dark:bg-blue-500/15 text-blue-700 dark:text-blue-400">{app.match}%</Badge>
          ...
          <CardContent className="space-y-3">
            <Link href="/resume" className="block">
              <div className="flex items-center gap-4 p-3 rounded-lg border hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors cursor-pointer">
                <div className="h-10 w-10 rounded-xl bg-blue-100 dark:bg-blue-500/10 flex items-center justify-center"><Upload className="h-5 w-5 text-blue-500" /></div>
                <div><p className="text-sm font-medium">Upload Resume</p>            <p className="text-xs text-gray-500 dark:text-gray-400">Let AI extract your skills and experience</p></div>
                <ArrowUpRight className="h-4 w-4 text-gray-400 ml-auto" />
              </div>
            </Link>
            <Link href="/jobs" className="block">
              <div className="flex items-center gap-4 p-3 rounded-lg border hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors cursor-pointer">
                <div className="h-10 w-10 rounded-xl bg-emerald-100 dark:bg-emerald-500/10 flex items-center justify-center"><Briefcase className="h-5 w-5 text-emerald-500" /></div>
                <div><p className="text-sm font-medium">Find Jobs</p><p className="text-xs text-gray-500 dark:text-gray-400">Browse AI-matched opportunities</p></div>
                <ArrowUpRight className="h-4 w-4 text-gray-400 ml-auto" />
              </div>
            </Link>
